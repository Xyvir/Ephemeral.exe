...
- **Every listed node is probe-verified** — a dial + hello handshake proves a node speaks the wire protocol, but not that it is a live compute node. So each refresh actually sends every reachable entry a real job (a tiny bash `echo` payload carrying a fresh per-node nonce) and records the verdict in `docs/swarm.json` (`probe: "ok"|"failed"|"unreachable"`, `probe_at`, `probe_detail`); a node is only ranked as verified when it executed the payload and echoed the nonce back, which a bot that merely answers hello cannot fake. The census keeps **only live members**: an entry that cannot be dialed, or that is dialed but fails its probe, is dropped from the list immediately — no failure counters, no tombstones, no blacklist. A node that comes back online (an intermittent laptop, a restarted VM) is simply re-discovered through its live peers on the next refresh. The refresh is **tiered**: the previous list regenerates itself from any reachable member; if none answers, the genesis anchor is dialed; if *nothing* at all is reachable (a GitHub-Actions network outage, the anchor down), the previous list is written back **verbatim as last good state** — held indefinitely, never wiped to empty — so a temporary outage can't cascade into a permanently empty address list. Run a manual check anytime with `python scripts/update_swarm_json.py --no-probe --out /tmp/swarm.json` (dial-only) or just watch the per-node lines in the action log.
...
- **Own the relays (org end-to-end)** — `EPHEMERAL_RELAY` accepts a comma-separated list of self-hosted relay URLs (`https://relay.myorg.com`) in addition to the `n0`/`minimal`/`disabled` presets; `scripts/setup_relay.sh` deploys the upstream `iroh-relay` binary as a bare systemd service (ACME TLS, optional endpoint allowlist/denylist; `RELAY_RUNTIME=podman` for container-first hosts) so an org can run the entire transport in-house — the relay protocol only exists in the Rust binary, so this is a binary, not a Python/wasm service. `EPHEMERAL_RELAY_FALLBACK=1` appends the public n0 relays to a custom list for failover (off by default — a private org's traffic stays off n0). Relay URLs flow through hello frames, `/ready`, and `docs/swarm.json` unchanged, so custom relays need no swarm-list changes — the list just records whichever relay each node reports.
...
* **Phase 3 — browser client & desktop tier:** the WebAssembly thin client and the desktop tiers (one unified front end, `ephemeral_ui/`, with `local` and `distributed` backends). Both desktop tiers build for Windows (EXE) and Linux (AppImage).

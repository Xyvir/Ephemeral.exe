...
- **DNS TXT redundancy (optional)** — when the list itself is unreachable (e.g. GitHub is down), first contact can still happen through **DNS**: the same scheduled Action keeps a TXT record that **mirrors the list** (one compact `iroh1:<node_id>;<relay>` entry per node, comma-separated and split across 255-char strings — tickets are too long for DNS and arrive via the hello handshake anyway), and every node *and* the wasm SPA resolve it via DNS-over-HTTPS as a final fallback. DNS is tiered, cached infrastructure, so it's an independent path to the swarm. Configure it in the repo settings: `EPHEMERAL_DNS_TXT` (variable — the TXT record hostname) + `EPHEMERAL_DNS_TOKEN` (secret — a Cloudflare API token with DNS edit); `EPHEMERAL_DNS_ZONE` is optional and auto-detected. Runtime falls back to `EPHEMERAL_DNS_TXT` (env) automatically — no code changes. The mirror is capped at ~30 entries to stay well under DNS size limits — dialing any one of them reveals the whole swarm via `hello`.

...
**Discovery is automagic.** On load the client fetches the **live swarm list** (`docs/swarm.json`, refreshed every 6 h by a scheduled GitHub Action), dials the current members by **stable node id + relay** (iroh-native — tickets only as a fallback for legacy peers), and falls back to the compiled seed nodes in `web/config.js` — this is configuration, not a job-routing dependency, so the execution path still runs entirely over the iroh network with no HTTP endpoint. Each dial completes the `hello` handshake and learns the whole cluster from the seed's peer table (ids, relays, tickets and warm images); discovery runs concurrently so a few dead nodes can't stall it. Jobs then route automatically to the best available compute node — a peer whose warm images cover the document's languages first, then lowest RTT — submitted by node id + relay through the same `submit_job_to_node` path as the Python tiers. A manual seed-ticket field remains as an override for operators, and the *Cluster* panel lists discovered nodes with their images and latency.

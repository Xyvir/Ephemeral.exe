...
- **DNS TXT redundancy (optional)** — when the list itself is unreachable (e.g. GitHub is down), first contact can still happen through **DNS**: the same scheduled Action can keep a TXT record (`iroh1:<node_id>;<relay>` — the always-on anchor) in sync with the list, and every node *and* the wasm SPA resolve it via DNS-over-HTTPS as a final fallback. DNS is tiered, cached infrastructure, so it's an independent path to the swarm. Configure it in the repo settings: `EPHEMERAL_DNS_TXT` (variable — the TXT record hostname) + `EPHEMERAL_DNS_TOKEN` (secret — a Cloudflare API token with DNS edit); `EPHEMERAL_DNS_ZONE` is optional and auto-detected. Runtime falls back to `EPHEMERAL_DNS_TXT` (env) automatically — no code changes. One anchor is enough: its `hello` handshake reveals the whole swarm.

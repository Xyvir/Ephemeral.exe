...
* **Desktop / Windows users:** just run **`Ephemeral-Distributed.exe`** — it runs its own compute node when no background service is installed, or acts as a thin front-end to the installed **Background Service** (one node per machine). For a truly always-on node, right-click the tray icon and choose **Install Background Service**: it registers `Ephemeral-Distributed.exe --service` as a boot-time scheduled task (running as SYSTEM), so your machine keeps serving jobs even while you're logged off.

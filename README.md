...
- **Every listed node is probe-verified** — a dial + hello handshake proves a node speaks the wire protocol, but not that it is a live compute node. So each refresh actually sends every reachable entry a real job (a tiny Python `print` payload carrying a fresh per-node nonce) and records the verdict in `docs/swarm.json` (`probe: "ok"|"failed"|"unreachable"`, `probe_at`, `probe_detail`); a node is only ranked as verified when it executed the payload and echoed the nonce back, which a bot that merely answers hello cannot fake. Entries that are reachable but never run the probe are evicted after 3 failed probes; entries that go silent are kept for ~36 h (6 runs) in case they're just offline — but only if they've ever been seen alive (a node that has never once answered a dial has no recovery to wait for, so it ages out after just 2 runs, ~12 h). The genesis anchor is exempt from eviction. Run a manual check anytime with `python scripts/update_swarm_json.py --no-probe --out /tmp/swarm.json` (dial-only) or just watch the per-node lines in the action log.

...
The desktop tray clients (`main_local.py` local-only, `main_distributed_client.py` distributed) are cross-platform: the same code builds a Windows EXE (PyInstaller) and a Linux **AppImage** (PyInstaller onedir + appimagetool, via `packaging/build_appimage.sh local|distributed`). Platform plumbing is guarded: the language prompt uses zenity/kdialog/tkinter on Linux, image-clipboard uses wl-copy/xclip, login autostart writes a `~/.config/autostart/ephemeral.desktop` entry, and Podman lifecycle uses the native rootless socket (`systemctl --user start podman.socket`) instead of `podman machine`. Both apps also expose `--cli script.md` (headless) and `--self-check` (install verification) modes. On Windows, the distributed tray's **Background Service** menu item (shown checked while installed) toggles a boot-time scheduled task that runs `Ephemeral-Distributed.exe --service` as SYSTEM from its own permanent copy under `%LOCALAPPDATA%` (a separate file from the **Install && Run on Boot** copy, so the two features are independent — enabling startup while the service is installed works, and uninstalling the service never touches the autostart entry). The node stays in the swarm even while the user is logged off (the tray app keeps acting as the front end while logged in). The distributed tray uses a **merged identity** — one node per machine: when the Background Service is installed, the tray acts as a thin front-end to it (jobs run through the always-on node, so every user's tray on the box shares the one identity and the one warm image cache; the service's state lives in the shared `%PUBLIC%` folder and its localhost API port is auto-picked when taken, with the winner persisted for tray discovery). If the service is unreachable at job time, the tray falls back to running its own node for that job — no admin required either way.
